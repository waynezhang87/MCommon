...
public class ImageLoader {
                ...
                public void handleMessage(Message msg) {
                    ...
                    if (imageView.getTag().toString().equals(path)) {
                        if(bm != null){
                            ...
                            imageView.setTag(R.id.mc_image_item, false);
                        } else {
                            //表示图已经损坏
                            L.e(TAG, "Image[" + path + "] is damaged, can not load.");
                            imageView.setTag(R.id.mc_image_item, true);
                            cache.putCache(CacheKey.IMAGE_DAMAGED_PREFIX + path, "1");
                        }
                    }
                }
            };
    ...
    private ImageSize getImageViewWidth(ImageView imageView) {
        ...
        int width = params.width == ViewGroup.LayoutParams.WRAP_CONTENT ? 0 : imageView.getWidth(); // Get
        // actual image width
        if (width <= 0)
        ...
        int height = params.height == ViewGroup.LayoutParams.WRAP_CONTENT ? 0 : imageView.getHeight(); // Get
        // actual image height
        if (height <= 0)
